...
def train_val_NN(net, phase, train_loader, val_loader, optimizer, scheduler, criterion,
                 train_size, val_size, batch_size, n_epochs, device, model_savepath,
    ...
    for epoch in range(n_epochs):  # loop over the dataset multiple times
        ...
        for i, sample_batched in enumerate(data_loader, 0):
            ...
            with torch.set_grad_enabled(phase == 'train'):  # comparable to torch.no_grad()
                ...
                # Multi-class
                # loss = cosineLoss(outputs, labels_batch, n_classes=2)
            ...
            # Multi-class
            # prob_outputs = nn.Softmax(dim=1)(outputs.detach())[:, 1].numpy()
...
##
def modelOutput(iepoch, net, test_loader, test_size, positive_prob_treshold, model_savepath):
    net.eval()
...
if __name__ == '__main__':
    ...
    result_savepath = './TLClassifier_result'
    ...
    #  TODO: add class weight to loss function
    #   reduction='none', : https://discuss.pytorch.org/t/dealing-with-imbalanced-datasets-in-pytorch/22596
    #   add an attribute to dataloader, each batch
    ...
    #                  train_size, train_size, batch_size, n_epochs, device, model_savepath,
    #                  use_manual_reg, LAMBDA_conv, LAMBDA_fc, fc_reg_form, use_fc_weight_clip, SparsificationFactor,
    #                  positive_prob_treshold, metric_names, name_funcs_dict)
    ...
    ytest, yscore, yprob, ypred = \
        modelOutput(optimal_epoch, net, val_loader, val_size, positive_prob_treshold, model_savepath)
