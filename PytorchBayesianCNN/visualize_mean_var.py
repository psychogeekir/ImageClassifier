...
def draw_distributions(filename, save_dir, type='mean', node_no=0, save_plots=False, plot_time=0.5):
    ...
    if type=='both':
        ...
        for i in range(len(means)):
            mean = np.mean(means[i].reshape((means[i].shape[0], -1))[:, node_no])
            std = np.sum(np.square(stds[i].reshape((stds[i].shape[0], -1))[:, node_no])) / stds[i].shape[0]
        ...
        for i in range(len(data)):
            sample = data[i].reshape((data[i].shape[0], -1))
...
def draw_lineplot(filename, save_dir, type='mean', node_no=0, save_plots=False, plot_time=5):
    ...
    batch_size = file_desc['batch_size']
    freq = file_desc['recording_frequency_per_epoch']
    means, stds = utils.load_mean_std_from_file(filename)  # list of reshaped np.array()
    data = means if type=='mean' else stds
    ...
    plt.pause(plot_time)
    if save_plots:
        plt.savefig(save_dir + '/' + f'{layer}-node_{node_no}-{type}-lineplot.png')
    return None
